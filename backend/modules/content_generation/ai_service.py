...
"""
        ...
        # Add focused section based on content_focus
        if content_focus != "all":
            user_prompt += f"\n**CONTENT FOCUS - EMPHASIZE THIS:**\n"

            if content_focus == "recent_posts":
                if linkedin_data.get('posts'):
                    user_prompt += "Focus on their recent LinkedIn posts and activity:\n"
                    for i, post in enumerate(linkedin_data['posts'][:3]):
                        post_content = post.get('content', post) if isinstance(post, dict) else str(post)
                        user_prompt += f"- Post {i+1}: {post_content[:200]}\n"

            elif content_focus == "skills":
                if linkedin_data.get('skills'):
                    user_prompt += f"Focus on their skills and expertise:\n"
                    user_prompt += f"- Skills: {', '.join(linkedin_data['skills'][:10])}\n"

            elif content_focus == "recommendations":
                if linkedin_data.get('recommendations'):
                    user_prompt += "Focus on their LinkedIn recommendations:\n"
                    for i, rec in enumerate(linkedin_data['recommendations'][:3]):
                        rec_text = rec.get('text', rec) if isinstance(rec, dict) else str(rec)
                        user_prompt += f"- Recommendation {i+1}: {rec_text[:200]}\n"

            elif content_focus == "geography":
                if linkedin_data.get('location'):
                    user_prompt += f"Focus on their geographic location and regional context:\n"
                    user_prompt += f"- Location: {linkedin_data['location']}\n"
                    user_prompt += "Emphasize local presence, regional expertise, or location-specific value.\n"

            elif content_focus == "about":
                if linkedin_data.get('about'):
                    user_prompt += f"Focus on their profile description/about section:\n"
                    user_prompt += f"- About: {linkedin_data['about']}\n"

            elif content_focus == "education":
                if linkedin_data.get('education'):
                    user_prompt += "Focus on their educational background:\n"
                    for edu in linkedin_data['education'][:3]:
                        if isinstance(edu, dict):
                            edu_text = f"{edu.get('degree', '')} {edu.get('field', '')} - {edu.get('school', '')}"
                        else:
                            edu_text = str(edu)
                        user_prompt += f"- {edu_text}\n"

            elif content_focus == "experience":
                if linkedin_data.get('experience'):
                    user_prompt += "Focus on their work experience and career journey:\n"
                    for exp in linkedin_data['experience'][:3]:
                        if isinstance(exp, dict):
                            exp_text = f"{exp.get('title', '')} at {exp.get('company', '')}"
                        else:
                            exp_text = str(exp)
                        user_prompt += f"- {exp_text}\n"

            user_prompt += "\n**IMPORTANT:** The content MUST specifically reference and center around the above focused information.\n"
